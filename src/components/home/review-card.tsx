...
import { AVATAR_COLORS } from "@/app/api/utils/avatarColors";
...
export function ReviewCard({
  ...
  const ac = AVATAR_COLORS[(pageStart + index) % AVATAR_COLORS.length];
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      {/* Review text */}
      <p className="line-clamp-4 flex-1 text-xs leading-relaxed text-gray-500">
        {review.description}
      </p>
      ...
      <a
        ...
        className="mt-auto text-xs font-medium text-blue-500 hover:text-blue-600 hover:underline"
      >
        Read on Google →
      </a>
    </div>
